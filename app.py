...
# 🟢 AREA B: 껄무새 놀이터
    ...
    if fg_index < 40:
        ...
        fg_advice = "시장 전체가 매수 적기로 진입 중입니다. 흐린 눈 소비를 조금만 아껴서 우량 자산을 모으면 미래 자산이 퀀텀점프합니다."
        signal_led = "🟢 예수금 충전 라이트 온"
    elif fg_index > 65:
        fg_status = "🚀 극단적 탐욕 (Extreme Greed)"; fg_color = "#81C995"
        fg_advice = "포모(FOMO)에 밀려 고점에 추격 매수하기 쉬운 날입니다. 충동적인 진입을 잠시 멈추고 관망하는 것이 유리합니다."
        signal_led = "🔴 충동 매수 제어 경보 발령"
    else:
        fg_status = "📊 중립 기어 (Neutral)"; fg_color = "#8AB4F8"
        fg_advice = "안정적인 시장 기류입니다. 평소의 불필요한 지출을 점검하고 정기 적립식 매수를 이어가기 적절합니다."
                ...
                if missed_money > 0:
                    if selected_option == "✍️ 내 쓸모없는 지출 직접 입력하기":
                        if total_seed <= 3000000:
                            ...
                            card_info = {"title": "🚨 소비 광기 야수 대리님", "desc": f"웬만한 중형 신차 한 대를 풀옵션으로 뽑고도 남을 거금을 오직 '{habit_clean_name}'에 하수구 물 버리듯 쏟아부으셨네요.\n미래에 {asset_clean_name}의 강력한 상승 기류를 타고 조기 은퇴할 수 있었던 프리패스 티켓을 본인 손으로 과감하게 찢어발기셨습니다.\n내일 아침 출근 버스 창가에 머리를 대고 오늘 확인한 손실 리포트를 깊게 복습해 보시기 바랍니다."}
                        else:
                        ...
                        card_info = custom_cards.get(selected_option, {"title": "🛍 Honor 시발비용 매니저", "desc": "눈앞의 자잘한 소비에 영혼을 지배당해 미래 자산 퀀텀점프의 황금 노선 버스를 쿨하게 놓치신 철없는 직장인.\n남들 은퇴해서 휴양지에서 파이어족으로 호의호식할 때 님은 환갑 넘어서도 출근 셔틀버스 안에서 이거 켜고 피눈물 흘릴 운명입니다."})
                else:
...
"""
        ...
        with tab4:
            ...
            if st.session_state["current_audit"] is None:
                ...
                fomo_advice = CUSTOM_FOMO_MAP.get(habit_name, "금융 시장의 격렬한 변동성에 밀려 눈물 흘리느니, 내 신체 건강과 정신적 행복에 100% 자율 투자한 귀하가 마인드셋 최종 승리자입니다.")
        ...
        flash_news = [
            f"⚡ [소비 헷지 살] 월급 루팡 개미들, 장바구니 풀소유로 미장 빅테크 시총 방어 대리 기여 포착",
            f"⚡ [도파민 랠리] 2030 직장인 무리, 분초 단위로 충동 결제 누르며 스트레스 사수 기믹 연출",
            f"⚡ [긴급 정산] 하이닉스 주가 폭발 랠리 속, 출근길 커피 결제단의 예수금 동결 위기 감지",
            f"⚡ [사주도사 소견] 금일 투자 타이밍이 불확실한 야수들 속출, 소비 우회 방어가 정신 건강에 유리하다 판단"
        ]
