...
st.set_page_config(
    page_title="✨ 껄무새 - 자산 최적화 시뮬레이션 v15", 
    page_icon="🦜",
    layout="centered" 
)
...
# 🟢 AREA B: 껄무새 팩폭 + 2030 남녀 도파민 놀이터
else:
    # 📊 2030 남녀 취향 저격 지출 항목 풀
    HABIT_PRICE_DICT = {
    ...
    # 🛠️ [타이틀 개조] 세련된 무지개빛 그라데이션 스타일의 한글 "껄무새" 로고 배정!
    st.markdown("""
        <div style="margin-bottom: 10px;">
            <span style="font-size: 36px; font-weight: 800; background: linear-gradient(45deg, #4285F4, #9B51E0, #E91E63); -webkit-background-clip: text; -webkit-text-fill-color: transparent; letter-spacing: -1px;">
                🦜 껄무새
            </span>
        </div>
        <h2 style="font-family: sans-serif; font-weight: 400; color: #FFFFFF; margin-top: 0; margin-bottom: 25px; font-size: 23px;">
            그때 살 걸... 귀하의 흐린 눈 지출을 추적해 자산 최적화 대안을 제시합니다.
        </h2>
    ...
    birth_month = st.selectbox("본인의 태어난 음력/양력 달(月)을 고르세요", [f"{m}월 생" for m in range(1, 13)])
    
    if st.button("☯️ 사주 오행 열어보기"):
        saju_database = {
    ...
    st.markdown("**Q. 평생 동안 {N}년 간 둘 중 딱 하나만 안 먹고 안 쓰고 주식 살 수 있다면?**")
    
    col_v1, col_v2 = st.columns(2)
    with col_v1:
        if st.button("🅰️ 평생 불금 배달 엽떡+치킨 세트 참기"): st.session_state["vote_data"]["A"] += 1
    with col_v2:
        if st.button("🅱️ 평생 올리브영 세일 '구경만' 가기 참기"): st.session_state["vote_data"]["B"] += 1
    ...
    st.caption(f"📊 실시간 주주 투표 현황: 🅰️ {per_A:.1f}% vs 🅱️ {per_B:.1f}% (총 {total_votes}명 참여)")
    ...
    with st.form("gemini_form"):
        ...
        submitted = st.form_submit_button("✨ 껄무새 타임머신 가동 (Enter)")
    ...
    if submitted:
        ...
        if missed_money > 0:
            custom_cards = {
                "탕후루/마라탕 수명 단축 쿨타임 (1회 18,000원)": {"title": "🩸 혈당 폭발 마라탕 중독자", "desc": "마라 국물 and 설탕 코팅에 영혼을 저당 잡아 혈당을 올리는 사이, 본인의 시드머니는 주식 시장에서 완전히 녹아내리게 방치한 위대한 푸드 파이터", "bg": "linear-gradient(135deg, #8B0000 0%, #1A1D20 100%)"},
    ...
    with st.container():
        user_comment = st.text_input("💬 흑우로서 한마디 남기기 (Enter 입력 시 등록)", placeholder="예: 축구 유니폼 살 돈 모았으면 이미 맨시티 구단주였음..")
        if user_comment:
